...
Component({
    ...
    methods: {
        init: function () {
            ...
            _session_id = self.data.session_id;
            _appid      = self.data.appid;
            if(_session_id.length < 20){
                setTimeout(()=>{
        ...
        showSelectRegion: function(boo) {
            let self = this;
            if (_objs._partition_quering || _objs._roles_quering) {
                clearTimeout(_objs._ptout);
                _objs._ptout = setTimeout(()=>{self.showSelectRegion(boo);}, 300);
                return;
            }
            if (_objs._need_partition_retry) {
                self.queryParitions({no_hide: !!boo});
                clearTimeout(_objs._ptout);
                _objs._ptout = setTimeout(()=>{self.showSelectRegion(boo);}, 300);
                return;
            }
        ...
        queryCommon: function (cmd, param, __cbk00__) {
            let self   = this;
            let key    = cmd + '_cnt';
            _objs[key] = _objs[key] || 0;
            _objs[key]++;
            if (_objs[cmd + '_querying'] || _session_id == '' || _session_id == 'nil') {
                if(_session_id == '' || _session_id == 'nil'){
                    self.triggerEvent('fress_session');
                    setTimeout(()=>{
                        self.queryCommon(cmd, param, __cbk00__);
                    }, 2000);
                }
                return;
            }
            ...
            wx.request({
                ...
                success: function (rs) {
                    var res = rs && typeof(rs) == 'object' ? rs : {};
                    if (res && ('' + res.ret === '-1702220400' || '' + res.ret === '-1702220407' || res.ret === 6211 || res.ret === 6204)) {
                        self.triggerEvent('updateSessionId');
                        _objs[cmd + '_querying'] = true;
                        return;
                    }
        ...
        queryParitions: function (par, __cbk21__) {
            ...
            if (_objs._partition_quering){return;}
            ...
            self.getReportObj(function (res) {
                ...
                self.queryCommon('partitionlist', param.join('&'), function (rs0) {
                    ...
                    if (typeof (__cbk21__) == 'function') {
                        ...
                        self.renderPartitions(rs0, params);
                    }
                });
            });
        },
